...
<section class="tp-hero">
    <div style="max-width:1200px; margin:0 auto;">
        ...
        <div class="tp-tab-bar">
            ...
            @if($profile->portfolio_items && count($profile->portfolio_items) > 0)
                    ...
                    <div style="display:grid; grid-template-columns:repeat(auto-fill, minmax(150px, 1fr)); gap:0.75rem;">
                        @foreach($profile->portfolio_items as $item)
                            @if(isset($item['type']) && $item['type'] === 'image')
                                <div style="aspect-ratio:1; border-radius:12px; overflow:hidden; background:rgba(255,255,255,0.04); border:1px solid rgba(255,255,255,0.07);">
                                    <img src="{{ $item['url'] }}" alt="Portfolio" style="width:100%; height:100%; object-fit:cover;">
                                </div>
                            @elseif(isset($item['type']) && $item['type'] === 'link')
                                <a href="{{ $item['url'] }}" target="_blank" rel="noopener"
                                   style="aspect-ratio:1; border-radius:12px; background:rgba(255,255,255,0.04); border:1px solid rgba(255,255,255,0.07); display:flex; flex-direction:column; align-items:center; justify-content:center; gap:6px; text-decoration:none; transition:border-color 0.15s;" onmouseover="this.style.borderColor='rgba(255,107,53,0.4)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.07)'">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" fill="none" stroke="#FF6B35" stroke-width="2" viewBox="0 0 24 24"><path d="M22.54 6.42a2.78 2.78 0 0 0-1.95-1.96C18.88 4 12 4 12 4s-6.88 0-8.59.46a2.78 2.78 0 0 0-1.95 1.96A29 29 0 0 0 1 12a29 29 0 0 0 .46 5.58A2.78 2.78 0 0 0 3.41 19.54C5.12 20 12 20 12 20s6.88 0 8.59-.46a2.78 2.78 0 0 0 1.95-1.96A29 29 0 0 0 23 12a29 29 0 0 0-.46-5.58z"/><polygon points="9.75 15.02 15.5 12 9.75 8.98 9.75 15.02"/></svg>
                                    <span style="color:rgba(255,255,255,0.4); font-size:0.7rem; font-weight:600;">Voir la vidéo</span>
                                </a>
                            @endif
                        @endforeach
                    </div>
