...
@section('content')
<div style="background:#FFFDF5; min-height:100vh; padding-bottom:4rem;">

    {{-- Hero --}}
    <div style="background:#FEF9E7; background-image:radial-gradient(ellipse 110% 65% at 50% 115%, rgba(217,119,6,0.12) 0%, transparent 65%); padding:3rem 1.5rem 0; border-bottom:1px solid #FDE68A;">
        <div style="max-width:860px; margin:0 auto;">
            ...
            @else
                <span style="display:inline-flex; align-items:center; gap:6px; background:rgba(217,119,6,0.08); border:1px solid rgba(217,119,6,0.3); color:#B45309; font-size:0.72rem; font-weight:800; padding:5px 14px; border-radius:100px; text-transform:uppercase; letter-spacing:0.08em; margin-bottom:1rem;">
                    <span style="width:6px; height:6px; border-radius:50%; background:#D97706; display:inline-block; animation:pulse 2s ease-in-out infinite;"></span>
                    Places disponibles
                </span>
            @endif

            <h1 style="font-size:clamp(1.8rem,4vw,2.6rem); font-weight:900; color:#1C1917; margin:0 0 1rem; letter-spacing:-0.02em;">
                {{ $experience->title }}
            </h1>

            {{-- Artiste --}}
            <div style="display:flex; align-items:center; gap:12px; margin-bottom:2rem;">
                <div style="width:44px; height:44px; border-radius:12px; background:linear-gradient(135deg,#D97706,#F59E0B); display:flex; align-items:center; justify-content:center; font-weight:900; font-size:1.1rem; color:white;">
                    {{ strtoupper(substr($experience->talentProfile->stage_name, 0, 1)) }}
                </div>
                <div>
                    <a href="{{ route('talent.show', $experience->talentProfile->slug) }}"
                       style="color:#1C1917; font-weight:800; font-size:0.95rem; text-decoration:none;">
            ...
            <div style="display:flex; flex-wrap:wrap; gap:10px; padding-bottom:2rem;">
                <div style="background:white; border:1px solid #FDE68A; border-radius:12px; padding:10px 16px; display:flex; align-items:center; gap:8px;">
                    <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" fill="none" stroke="#D97706" stroke-width="2" viewBox="0 0 24 24"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
                    <span style="color:#1C1917; font-size:0.875rem; font-weight:700;">{{ $experience->event_date->isoFormat('dddd D MMMM YYYY') }}</span>
                </div>
                <div style="background:white; border:1px solid #FDE68A; border-radius:12px; padding:10px 16px; display:flex; align-items:center; gap:8px;">
                    <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" fill="none" stroke="#D97706" stroke-width="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
                    <span style="color:#1C1917; font-size:0.875rem; font-weight:700;">{{ $experience->event_date->format('H\hi') }}</span>
                </div>
                <div style="background:white; border:1px solid #FDE68A; border-radius:12px; padding:10px 16px; display:flex; align-items:center; gap:8px;">
                    <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" fill="none" stroke="#D97706" stroke-width="2" viewBox="0 0 24 24"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>
                    <span style="color:#B45309; font-size:0.875rem; font-weight:800;">{{ number_format($experience->price_per_seat, 0, ',', '.') }} FCFA <span style="color:#9CA3AF; font-weight:600;">/ pers.</span></span>
                </div>
                <div style="background:white; border:1px solid #FDE68A; border-radius:12px; padding:10px 16px; display:flex; align-items:center; gap:8px;">
    ...
    <div style="max-width:860px; margin:0 auto; padding:2rem 1.5rem 0;">
        <div style="display:grid; grid-template-columns:1fr 360px; gap:2rem; align-items:start;">
            ...
            <div>
                ...
                <div style="background:white; border:1px solid rgba(217,119,6,0.15); border-radius:16px; padding:1.75rem; margin-bottom:1.5rem; box-shadow:0 1px 8px rgba(217,119,6,0.06);">
                    <h2 style="font-size:1rem; font-weight:800; color:#1C1917; margin:0 0 1rem; display:flex; align-items:center; gap:8px;">
                        <span style="display:block; width:3px; height:16px; border-radius:2px; background:#D97706;"></span>
                        Au programme
                    </h2>
                    <p style="color:#4B5563; font-size:0.9rem; line-height:1.8; margin:0; white-space:pre-line;">{{ $experience->description }}</p>
                </div>
                @endif

                {{-- Options premium --}}
                @if($experience->premium_options && count($experience->premium_options) > 0)
                <div style="background:white; border:1px solid rgba(217,119,6,0.2); border-radius:16px; padding:1.75rem; box-shadow:0 1px 8px rgba(217,119,6,0.06);">
                    <h2 style="font-size:1rem; font-weight:800; color:#1C1917; margin:0 0 1rem; display:flex; align-items:center; gap:8px;">
                        <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="none" stroke="#D97706" stroke-width="2" viewBox="0 0 24 24"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
                        Options Premium
                    </h2>
                    <div style="display:flex; flex-direction:column; gap:0.75rem;">
                        @foreach($experience->premium_options as $opt)
                        <div style="background:#FFFBEB; border:1px solid #FDE68A; border-radius:12px; padding:12px 16px; display:flex; align-items:center; justify-content:space-between;">
                            <div>
                                <p style="font-weight:700; color:#1C1917; font-size:0.88rem; margin:0;">{{ $opt['name'] ?? '' }}</p>
                                @if(!empty($opt['description']))
                                    <p style="color:#6B7280; font-size:0.78rem; margin:2px 0 0;">{{ $opt['description'] }}</p>
                                @endif
                            </div>
                            @if(!empty($opt['price']))
                                <span style="color:#B45309; font-weight:800; font-size:0.9rem; white-space:nowrap;">
            ...
            <div style="position:sticky; top:1.5rem;">
                <div style="background:white; border:1px solid rgba(217,119,6,0.2); border-radius:20px; padding:1.75rem; box-shadow:0 4px 20px rgba(217,119,6,0.08);">
                    ...
                    @elseif($myBooking && $myBooking->status->value !== 'cancelled')
                        ...
                        <div style="text-align:center; margin-bottom:1.25rem;">
                            ...
                            <h3 style="color:#1C1917; font-weight:900; font-size:1rem; margin:0 0 0.25rem;">Vous êtes inscrit !</h3>
                    ...
                    @else
                        ...
                        <h3 style="color:#1C1917; font-weight:900; font-size:1rem; margin:0 0 0.25rem;">Réserver ma place</h3>
                                ...
                                <form action="{{ route('client.meet-and-greet.book') }}" method="POST">
                                    ...
                                    <div style="margin-bottom:1.25rem;">
                                        ...
                                        <select name="seats_count"
                                                style="width:100%; padding:12px 16px; border:1.5px solid #FDE68A; border-radius:12px; background:#FFFBEB; color:#1C1917; font-size:0.9rem; font-weight:700; font-family:inherit; outline:none;">
                                    ...
                                    <div style="background:#FFFBEB; border:1px solid #FDE68A; border-radius:12px; padding:12px 16px; margin-bottom:1.25rem;">
                                        ...
                                        <p style="font-size:1.5rem; font-weight:900; color:#B45309; margin:0; line-height:1.1;">
                                            {{ number_format($experience->price_per_seat, 0, ',', '.') }}
                                            <span style="font-size:0.85rem; font-weight:700; color:#D97706;">FCFA</span>
                                        </p>
                                    </div>

                                    <button type="submit"
                                            style="width:100%; padding:14px; border-radius:14px; background:linear-gradient(135deg,#D97706,#F59E0B); color:white; font-weight:900; font-size:1rem; border:none; cursor:pointer; font-family:inherit; box-shadow:0 6px 20px rgba(217,119,6,0.35); transition:transform 0.15s, box-shadow 0.15s;"
                                            onmouseover="this.style.transform='scale(1.02)';this.style.boxShadow='0 10px 30px rgba(217,119,6,0.45)'"
                                            onmouseout="this.style.transform='';this.style.boxShadow='0 6px 20px rgba(217,119,6,0.35)'">
                                        🎟 Réserver ma place
                                    </button>
                                </form>
                            @else
                                <p style="color:#6B7280; font-size:0.85rem; text-align:center; margin:0;">Seuls les clients peuvent réserver.</p>
                            @endif
                        @else
                            <a href="{{ route('login') }}"
                               style="display:block; text-align:center; padding:14px; border-radius:14px; background:linear-gradient(135deg,#D97706,#F59E0B); color:white; font-weight:900; font-size:1rem; text-decoration:none; box-shadow:0 6px 20px rgba(217,119,6,0.35);">
                                Connexion pour réserver
                            </a>
                            <a href="{{ route('register') }}"
                               style="display:block; text-align:center; margin-top:10px; padding:12px; border-radius:12px; background:#FFFBEB; border:1.5px solid #FDE68A; color:#B45309; font-weight:700; font-size:0.875rem; text-decoration:none;">
                    ...
                    {{-- Garanties --}}
                    <div style="margin-top:1.5rem; padding-top:1.25rem; border-top:1px solid #FDE68A; display:flex; flex-direction:column; gap:8px;">
